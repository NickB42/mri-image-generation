...
class BraTSSliceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Context: neighboring slices at z-1, z-2, ..., z-slice_radius
        context_slices = []
        for dz in range(-self.slice_radius, 0):  # e.g. -2, -1 when slice_radius=2
            zz = z + dz
            for vol in vols:
                slice_2d = vol[:, :, zz]
                context_slices.append(self._preprocess_slice(slice_2d))
        x_context = torch.cat(context_slices, dim=0)  # (4 * slice_radius, H, W)
