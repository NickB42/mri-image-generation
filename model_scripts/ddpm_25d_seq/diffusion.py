...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, x, t, z_pos, context=None):
        """
        ...
        eps_theta = self.model(x, t, z_pos, context=context)
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, z_pos, context=None):
        """
        ...
        for i in reversed(range(self.timesteps)):
            ...
            img = self.p_sample(img, t, z_pos, context=context)

        return img

    @torch.no_grad()
    def sample(self, batch_size=16, z_pos=0.5, context=None):
        """
    ...
    @torch.no_grad()
    def ddim_sample_loop(self, shape, z_pos, context=None, sample_timesteps: int = 50, eta: float = 0.0):
        """
        ...
        for idx in range(len(steps)):
            ...
            eps = self.model(img, t, z_pos, context=context)
    ...
    @torch.no_grad()
    def sample_ddim(self, batch_size=16, z_pos=0.5, context=None, sample_timesteps: int = 50, eta: float = 0.0):
